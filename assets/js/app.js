/* r10: self-rating re-enabled; two-up layout kept; hardcoded excel load */
...
function updateVoiceList(){ const box=$('#dbgVoices'); if(!box) return; box.innerHTML=''; const list=(state.voices||[]).filter(v=> state.voicePanelTarget==='zh'? isZhVoice(v) : isDeVoice(v)); if(list.length===0){ box.innerHTML='<div class="meta">Keine passenden Stimmen gefunden.</div>'; return; }
  list.forEach(v=>{ const row=document.createElement('div'); row.className='voice'; const name=document.createElement('div'); name.className='name'; name.textContent=v.name||'(name)'; const meta=document.createElement('div'); meta.className='meta'; meta.textContent=`${v.lang||''} ${v.default?'· default':''}`; const actions=document.createElement('div'); actions.style.marginLeft='auto'; actions.style.display='flex'; actions.style.gap='6px'; actions.style.flexWrap='wrap';
    ...
    const act = (state.voicePanelTarget==='zh'? state.browserVoice.zh : state.browserVoice.de); if(act && (act.name===v.name || act.voiceURI===v.voiceURI)) name.textContent+='  •  [Aktiv]';
    actions.appendChild(pick); actions.appendChild(test); row.appendChild(name); row.appendChild(meta); row.appendChild(actions); box.appendChild(row); }); }
...
function doReveal(){ $('#solBox').classList.remove('masked'); state.revealedAt=Date.now(); const ttr=state.revealedAt-(state.startedAt||state.revealedAt); if(ttr>0){ state.session.ttrSum+=ttr; state.session.ttrCount+=1; }
  enableRating(); renderSessionStats(); }
...
function rate(mark){ // mark: 'known' | 'unsure' | 'unknown'
  ...
  renderSessionStats();
  // optionally store simple progress by lesson (seen count)
  try{
    const lessonKey = findLessonKeyOfCurrent();
    if(lessonKey){ const bl=state.progress.byLesson; bl[lessonKey]=bl[lessonKey]||{seen:0}; bl[lessonKey].seen=(bl[lessonKey].seen||0)+1; localStorage.setItem(LS_KEYS.progress, JSON.stringify(state.progress)); }
  }catch(e){}
  disableRating();
  // move on
  nextCard();
...
function findLessonKeyOfCurrent(){ // heuristic: find lesson that contains current reference
  // In diesem einfachen Ansatz suchen wir den Eintrag in den Lektionen, was O(n) ist, aber lokal ok.
  for(const [k,arr] of state.lessons.entries()){
    if(arr && arr.includes(state.current)) return k; 
  }
  return null;
}
...
window.addEventListener('DOMContentLoaded', ()=>{
  ...
  $('#fileImport').addEventListener('change', e=>{ stopAutoplayOnUserAction(); const f=e.target.files?.[0]; if(!f) return; const r=new FileReader(); r.onload=()=>{ try{ const p=JSON.parse(r.result); if(p && p.version==='v1'){ state.progress=p; localStorage.setItem(LS_KEYS.progress, JSON.stringify(state.progress)); populateLessonSelect(); alert('Fortschritt importiert.'); } else alert('Ungültiges Format.'); }catch(err){ alert('Import fehlgeschlagen: '+err.message); } }; r.readAsText(f); e.target.value=''; });
});

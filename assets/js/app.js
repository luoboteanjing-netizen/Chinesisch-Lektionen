/* v5.5 Browser-optimiert + Fixes r6
   - Datenquelle-Picker: ./data/index.json (falls vorhanden) ODER lokale Datei
   - Pro Sprache (DE/ZH) eigene Stimme + eigene Rate/Pitch-Regler
*/
...
const state={
  ...
  // globale Regler werden nicht mehr genutzt, bleiben aber zur Abwärtskompatibilität in settings
  rate:0.95, pitch:1.0,
  // pro Sprache
  ...
  settings:{
    excelPath:'./data/Long-Chinesisch_Lektionen.xlsx', mode:'de2zh', order:'random',
    rate:0.95, pitch:1.0, // legacy
    rateDe:0.95, pitchDe:1.0, rateZh:0.95, pitchZh:1.0,
    lessons:[], browserVoiceZh:null, browserVoiceDe:null, autoplayGap:800
  },
...
function updateVoiceList(){ const box=$('#dbgVoices'); if(!box) return; box.innerHTML='';
  const list=(state.voices||[]).filter(v=> state.voicePanelTarget==='zh'? isZhVoice(v) : isDeVoice(v));
  if(list.length===0){ box.innerHTML='<div class="meta">Keine passenden Stimmen gefunden.</div>'; return; }
  list.forEach(v=>{
    const row=document.createElement('div'); row.className='voice';
    const name=document.createElement('div'); name.className='name'; name.textContent=v.name||'(name)';
    const meta=document.createElement('div'); meta.className='meta'; meta.textContent=`${v.lang||''} ${v.default?'· default':''}`;
    const actions=document.createElement('div'); actions.style.marginLeft='auto'; actions.style.display='flex'; actions.style.gap='6px'; actions.style.flexWrap='wrap';

    const pick=document.createElement('button'); pick.className='btn'; pick.textContent='Diese Stimme wählen';
    pick.onclick=()=>{
      if(state.voicePanelTarget==='zh'){ state.browserVoice.zh=v; state.settings.browserVoiceZh=v.name||v.voiceURI; }
      else { state.browserVoice.de=v; state.settings.browserVoiceDe=v.name||v.voiceURI; }
      saveSettings(); updateVoiceList();
    };

    const test=document.createElement('button'); test.className='btn ghost'; test.textContent='Probehören';
    test.onclick=()=>{ const u=new SpeechSynthesisUtterance(state.voicePanelTarget==='zh'? '这是一个测试。' : 'Dies ist ein Test.'); u.lang=(state.voicePanelTarget==='zh')?'zh-CN':'de-DE'; u.voice=v; try{speechSynthesis.cancel();}catch(e){} speechSynthesis.speak(u); };
...
// Excel laden (aus URL/Path)
async function loadExcel(){ try{ EXCEL_URL = $('#excelPath').value || EXCEL_URL; const res=await fetch(EXCEL_URL,{cache:'no-store'}); const buf=await res.arrayBuffer(); await parseExcelBuffer(buf); state.settings.excelPath=EXCEL_URL; saveSettings(); }catch(e){ console.error('Excel konnte nicht geladen werden:',e); alert('Konnte Datei nicht laden: '+(e?.message||e)); } }

// Excel laden (lokale Datei)
async function loadExcelLocal(file){ try{ const buf=await file.arrayBuffer(); await parseExcelBuffer(buf); state.settings.excelPath='(lokal) '+(file.name||''); saveSettings(); $('#excelPath').value=state.settings.excelPath; }catch(e){ console.error('Excel (lokal) Fehler:',e); alert('Konnte lokale Datei nicht laden: '+(e?.message||e)); } }
...
function startTraining(){ const sel=$('#lessonSelect'); state.selectedLessons.clear(); const picked=[]; for(const o of sel.selectedOptions){ state.selectedLessons.add(o.value); picked.push(o.value); } state.settings.lessons=picked; saveSettings(); gatherPool(); if(!state.pool.length){ alert('Bitte zuerst Lektion(en) übernehmen.'); return; } state.idx = (state.order==='seq') ? 0 : null; if(state.order==='seq') setCard(state.pool[state.idx]); else setCard(state.pool[Math.floor(Math.random()*state.pool.length)]);
  try{ window.scrollTo({ top: 0, behavior: 'smooth' }); setTimeout(()=>{ window.scrollTo({ top: 0, behavior: 'smooth' }); }, 50); }catch(e){ window.scrollTo(0,0); setTimeout(()=>window.scrollTo(0,0), 50); }
}

function doReveal(){ $('#solBox').classList.remove('masked'); state.revealedAt=Date.now(); const ttr=state.revealedAt-(state.startedAt||state.revealedAt); if(ttr>0){ state.session.ttrSum+=ttr; state.session.ttrCount+=1; } renderSessionStats(); }
...
// === TTS core ===
const START_DELAY_MS=150;   // priming delay
const BETWEEN_DELAY_MS=800; // between word and sentence
let _ttsPrimed=false;
function ttsPrime(cb){ if(_ttsPrimed){ cb(); return; } setTimeout(()=>{ _ttsPrimed=true; cb(); }, START_DELAY_MS); }

function buildUtterance(text, langKey){ const lang=(langKey==='zh')?'zh-CN':'de-DE'; const u=new SpeechSynthesisUtterance(text||''); u.lang=lang; // per Sprache: Rate & Pitch
  if(langKey==='zh'){ u.rate=state.rateZh; u.pitch=state.pitchZh; } else { u.rate=state.rateDe; u.pitch=state.pitchDe; }
  const chosen=(langKey==='zh')?state.browserVoice.zh:state.browserVoice.de; if(chosen) u.voice=chosen; else { const L=(langKey==='zh')?'zh':'de'; const cand=(state.voices||[]).filter(v=>(v.lang||'').toLowerCase().startsWith(L)); u.voice=cand.find(v=>v.default)||cand[0]||null; } return u; }
...
// === Autoplay ===
function setAutoplay(on){ state.autoplay.on=on; if(!on){ try{ speechSynthesis.cancel(); }catch(e){} state.autoplay.timers.forEach(id=>clearTimeout(id)); state.autoplay.timers=[]; } updateAutoplayBtn(); }
...
// ==== Daten-Picker (./data/index.json) ====
async function openDataPicker(){ // versucht index.json zu laden
  try{
    const res=await fetch('./data/index.json',{cache:'no-store'});
    if(!res.ok) throw new Error('no index');
    const list=await res.json(); if(!Array.isArray(list)) throw new Error('bad json');
    const box=$('#dataList'); box.innerHTML='';
    list.filter(x=>/\.xlsx$/i.test(x)).forEach(name=>{
      const row=document.createElement('div'); row.className='file';
      const label=document.createElement('div'); label.textContent=name;
      const b=document.createElement('button'); b.className='btn'; b.textContent='Laden'; b.onclick=()=>{ $('#excelPath').value='./data/'+name; closeDataPicker(); loadExcel(); };
      row.appendChild(label); row.appendChild(b); box.appendChild(row);
    });
    $('#dataPanel').classList.remove('hidden');
  }catch(err){ // Fallback: lokaler Picker
    const inp=$('#fileExcel'); inp.value=''; inp.click();
  }
}
function closeDataPicker(){ $('#dataPanel').classList.add('hidden'); }
...
window.addEventListener('DOMContentLoaded', ()=>{
  ...
  // pro Sprache: Werte aus settings übernehmen
  ...
  // Excel initial laden
  $('#btnReloadExcel').addEventListener('click', ()=> { stopAutoplayOnUserAction(); openDataPicker(); });
  $('#btnCloseData').addEventListener('click', closeDataPicker);
  $('#btnPickLocal').addEventListener('click', ()=>{ $('#fileExcel').click(); });
  $('#fileExcel').addEventListener('change', e=>{ const f=e.target.files?.[0]; if(!f) return; closeDataPicker(); loadExcelLocal(f); });
  ...
  // Erste Daten laden (falls Standardpfad existiert)
  loadExcel().catch(()=>{});
});

/* Flashcards v5.1: Fixes
 - Auto-Play nur bei chinesischer Antwort (DE→ZH)
 - Countdown 15s
 - Antwort Pinyin sicher verdeckt bis Reveal (CSS + Maskenlogik)
 - POS (Wortart) = 16px; sonstige Fonts = 20px
 - ZH-Frage: Satz Hanzi + Pinyin; Buttons unter Karte
*/
...
const state = {
  ...
  revealTimer:null, countdownTimer:null, countdownStart:15,
};
...
function gatherPool(){
  const out=[]; for(const k of state.selectedLessons){ const arr=state.lessons.get(k); if(arr) out.push(...arr);} state.pool=out; state.idx=null;
}

function clearTimers(){ if(state.revealTimer){clearTimeout(state.revealTimer); state.revealTimer=null;} if(state.countdownTimer){clearInterval(state.countdownTimer); state.countdownTimer=null;} }

function setCard(entry){
  state.current=entry; clearTimers(); const solBox=$('#solBox'); solBox.classList.add('masked'); $('#countdown').textContent=String(state.countdownStart);
  ...
  let remain=state.countdownStart; state.countdownTimer=setInterval(()=>{ remain--; if(remain>=0) $('#countdown').textContent=String(remain); },1000);
  state.revealTimer=setTimeout(()=>{ doReveal(); }, state.countdownStart*1000);

  $('#btnNext').disabled=false; $('#btnReveal').disabled=false; $('#btnPlayQ').disabled=false; $('#btnPlayA').disabled=false; $('#btnPrev').disabled=(state.order!=='seq');
}

function doReveal(){ const solBox=$('#solBox'); solBox.classList.remove('masked'); clearTimers(); if(state.mode==='de2zh') playAnswer(); }
...
function playQuestion(){ if(!state.current) return; if(state.mode==='de2zh'){ const w=state.current.word.de||''; const s=state.current.sent.de||''; speak(w,'de-DE'); setTimeout(()=>speak(s,'de-DE'),700);} else { const w=state.current.word.zh||''; const s=state.current.sent.zh||''; speak(w,'zh-CN'); setTimeout(()=>speak(s,'zh-CN'),700);} }
function playAnswer(){ if(!state.current) return; if(state.mode==='de2zh'){ const w=state.current.word.zh||''; const s=state.current.sent.zh||''; speak(w,'zh-CN'); setTimeout(()=>speak(s,'zh-CN'),700);} else { const w=state.current.word.de||''; const s=state.current.sent.de||''; speak(w,'de-DE'); setTimeout(()=>speak(s,'de-DE'),700);} }

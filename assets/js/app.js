/* r16: Load all sheets except first, independent of sheet names; display names from A2 (trim first 2 chars); Option A (empty if too short) */
let EXCEL_URL = './data/Long-Chinesisch_Lektionen.xlsx';
const DATA_START_ROW=3; // headers above
...
const state={
  ...
  // progress.byLesson: { [lessonKey]: { known:number, unknown:number } }
...
async function parseExcelBuffer(buf){ const wb=XLSX.read(buf,{type:'array'}); state.lessons.clear();
  // load all sheets except the first one (index 0)
  for(let i=1;i<wb.SheetNames.length;i++){
    const shName=wb.SheetNames[i]; const sh=wb.Sheets[shName]; if(!sh) continue;
    const a2 = sh['A2']?.v ? String(sh['A2'].v) : '';
    const disp = (a2.length>=2? a2.substring(2): '').trim(); // Option A: empty if too short
    const entries=[];
...
function populateLessonSelect(){ const sel=$('#lessonSelect'); sel.innerHTML=''; const keys=Array.from(state.lessons.keys()).map(k=>parseInt(k,10)).sort((a,b)=>a-b);
...
function findLessonKeyOfCurrent(){ // find which lesson contains the current entry
  for(const [k,obj] of state.lessons.entries()){
    const arr=obj?.entries||[]; if(arr.includes(state.current)) return k;
  }
  return null;
}
...
window.addEventListener('DOMContentLoaded', ()=>{
  ...
  $('#btnClearLessons').addEventListener('click', ()=>{ stopAutoplayOnUserAction(); state.selectedLessons.clear(); state.settings.lessons=[]; saveSettings(); state.pool=[]; state.idx=null; resetSessionStats(); const sel=$('#lessonSelect'); for(const o of sel.options){ o.selected=false; } if(state.trainingOn) stopTraining(); });
